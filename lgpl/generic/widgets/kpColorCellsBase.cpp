...
{
public:
    explicit kpColorCellsBasePrivate(kpColorCellsBase *q)
        : q(q)
    {
        colors = nullptr;
        inMouse = false;
...
kpColorCellsBase::kpColorCellsBase(QWidget *parent, int rows, int cols)
    : QTableWidget(parent)
    , d(new kpColorCellsBasePrivate(this))
{
...
void kpColorCellsBase::setRowColumnCounts(int rows, int columns)
{
    ...
    qCDebug(kpLogColorCollection) << "oldRows=" << oldRows << "oldCols=" << oldCols << "newRows=" << newRows << "newCols=" << newCols;
#endif
...
int kpColorCellsBase::positionToCell(const QPoint &pos, bool ignoreBorders, bool allowEmptyCell) const
{
...
void kpColorCellsBase::mouseMoveEvent(QMouseEvent *e)
{
    ...
    if (d->inMouse) {
        ...
        if (e->x() > d->mousePos.x() + delay || e->x() < d->mousePos.x() - delay || e->y() > d->mousePos.y() + delay || e->y() < d->mousePos.y() - delay) {
            ...
            if (cell != -1) {
#if DEBUG_KP_COLOR_CELLS_BASE
                qCDebug(kpLogColorCollection) << "beginning drag from cell=" << cell << "color: isValid=" << d->colors[cell].isValid()
...
{
#if DEBUG_KP_COLOR_CELLS_BASE
    qCDebug(kpLogColorCollection) << "kpColorCellsBase::dragEnterEvent() acceptDrags=" << d->acceptDrags
...
{
#if DEBUG_KP_COLOR_CELLS_BASE
    qCDebug(kpLogColorCollection) << "kpColorCellsBase::dragMoveEvent() acceptDrags=" << d->acceptDrags
                                  << " canDecode=" << KColorMimeData::canDecode(event->mimeData());
#endif
...
void kpColorCellsBase::dropEvent(QDropEvent *event)
{
    ...
    const int dragSourceCell = event->source() == this ? positionToCell(d->mousePos, true) : -1;
#if DEBUG_KP_COLOR_CELLS_BASE
    qCDebug(kpLogColorCollection) << "kpColorCellsBase::dropEvent()"
                                  << "color: rgba=" << (const int *)c.rgba() << "isValid=" << c.isValid() << "source=" << event->source()
...
#if DEBUG_KP_COLOR_CELLS_BASE
        qCDebug(kpLogColorCollection) << "\tdropAction=" << event->dropAction() << "destOldColor.rgba=" << (const int *)destOldColor.rgba();
#endif
