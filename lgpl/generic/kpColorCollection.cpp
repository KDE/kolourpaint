...
QStringList kpColorCollection::installedCollections()
{
...
{
    KMessageBox::error(parent, i18n("Could not open color palette \"%1\".", kpUrlFormatter::PrettyFilename(url)));
}

// TODO: Set d->editable?
bool kpColorCollection::open(const QUrl &url, QWidget *parent)
{
    if (url.isEmpty()) {
        return false;
    }
...
{
    KMessageBox::error(parent, i18n("Could not open KDE color palette \"%1\".", name));
}

bool kpColorCollection::openKDE(const QString &name, QWidget *parent)
{
#if DEBUG_KP_COLOR_COLLECTION
...
static void CouldNotSaveDialog(const QUrl &url, QWidget *parent)
{
    ...
    KMessageBox::error(parent, i18n("Could not save color palette as \"%1\".", kpUrlFormatter::PrettyFilename(url)));
}
...
bool kpColorCollection::saveAs(const QUrl &url, QWidget *parent) const
{
    if (url.isLocalFile()) {
...
kpColorCollection &kpColorCollection::operator=(const kpColorCollection &p)
{
    if (&p == this)
        return *this;
...
QColor kpColorCollection::color(int index) const
{
...
int kpColorCollection::findColor(const QColor &color) const
{
...
QString kpColorCollection::name(int index) const
{
    if ((index < 0) || (index >= count()))
        return {};
...
int kpColorCollection::addColor(const QColor &newColor, const QString &newColorName)
{
    d->colorList.append(ColorNode(newColor, newColorName));
    return count() - 1;
}

int kpColorCollection::changeColor(int index, const QColor &newColor, const QString &newColorName)
{
    if ((index < 0) || (index >= count()))
        return -1;
...
int kpColorCollection::changeColor(const QColor &oldColor, const QColor &newColor, const QString &newColorName)
{
