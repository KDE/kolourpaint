...
#
# Changes only those .h/.cpp pairs where the .h file contains a base clase.
# (derives from nothing).

HEADERS_WITH_ONE_BASE_CLASS=$((for f in `find -name \*.h | egrep -v '[A-Z]'`; do echo -n $f: ; egrep '^class kp[^:;]+$' $f | wc -l; done) | fgrep  :1 | cut -d: -f1 )

echo $HEADERS_WITH_ONE_BASE_CLASS

for f in $HEADERS_WITH_ONE_BASE_CLASS
do
